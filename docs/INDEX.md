# Documentation Index

Status: Active  
Owner: Founder  
Last Updated: 2026-04-07  
Purpose: Fast routing map for PrepHatch documentation.  
Audience: Founder, collaborators, and AI coding agents.

## Read This First

1. `docs/MASTER_BLUEPRINT.md`
2. `docs/DECISIONS.md`
3. `docs/product/mvp-scope.md`
4. `docs/architecture/system-architecture.md`
5. `docs/engineering/IMPLEMENTATION_BACKLOG.md`

## Core Docs

- `docs/MASTER_BLUEPRINT.md`: top-level source of truth
- `docs/DECISIONS.md`: accepted decisions and superseded history
- `docs/INDEX.md`: this file

## Product

- `docs/product/vision.md`
- `docs/product/mvp-scope.md`
- `docs/product/user-flows.md`
- `docs/product/pricing-and-access.md`

## Architecture

- `docs/architecture/system-architecture.md`
- `docs/architecture/tech-stack.md`
- `docs/architecture/data-model.md`
- `docs/architecture/exam-engine.md`
- `docs/architecture/auth-and-roles.md`
- `docs/architecture/payments-and-entitlements.md`
- `docs/architecture/ai-strategy.md`
- `docs/architecture/security.md`

## Engineering

- `docs/engineering/IMPLEMENTATION_BACKLOG.md`
- `docs/engineering/repo-structure.md`
- `docs/engineering/coding-standards.md`
- `docs/engineering/api-spec.md`
- `docs/engineering/testing-strategy.md`
- `docs/engineering/devops.md`
- `docs/engineering/migrations.md`

## Operations

- `docs/operations/content-ops.md`
- `docs/operations/support-runbook.md`
- `docs/operations/incident-runbook.md`
- `docs/operations/release-checklist.md`
- `docs/operations/qa-checklist.md`
- `docs/operations/cost-control.md`

## Internal

- `docs/internal/ai-prompts.md`

## Archived or Removed

- Redundant planning docs were folded into `docs/engineering/IMPLEMENTATION_BACKLOG.md`.
- Legacy prompt templates were merged into `docs/internal/ai-prompts.md`.
